...
public class LessThanTest {
    ...
    public void makeBranch_string_lit_gt_string_lit() throws  Exception{
        try{
            ...
            assertEquals(Token.LESS_THAN,token.getType());
    ...
    public void makeBranch_field_lpad() throws  Exception{
        try{
            Lexor tokenizer = new Lexor();
            tokenizer.setScript("FIELD1 < LPAD(FIELD1,2,'.')");
            tokenizer.parse();
            TokenTreeFactory tokenTreeFactory = new TokenTreeFactory();
            tokenTreeFactory.setListToken(tokenizer.getTokenList());
            tokenTreeFactory.make();
            ...
            Token lpad_length = tokenTreeFactory.getExecutionPlan().get(0).getChilds().get(1).getChilds().get(1);
            assertEquals(Token.NUMERIC_LITERAL,lpad_length.getType());

            Token lpad_pad = tokenTreeFactory.getExecutionPlan().get(0).getChilds().get(1).getChilds().get(2);
    ...
    public void makeBranch_field_concat() throws  Exception{
        try{
            ...
            Token concat_field2 = tokenTreeFactory.getExecutionPlan().get(0).getChilds().get(1).getChilds().get(0);
    ...
    public void makeBranch_field_substr() throws  Exception{
        try{
            Lexor tokenizer = new Lexor();
            tokenizer.setScript("FIELD1 < SUBSTR(FIELD2,2,5)");
            tokenizer.parse();
            TokenTreeFactory tokenTreeFactory = new TokenTreeFactory();
            tokenTreeFactory.setListToken(tokenizer.getTokenList());
            tokenTreeFactory.make();
